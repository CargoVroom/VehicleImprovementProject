...
Roguetech uses a heavily customized and expanded version of this mod. Using them together is probably a bad idea.
...
Gave Scimitar-B it's missing SRM4,
Fixed a typo in resizer settings.
Removed "Light" tag from Hunters.

1.0.9

Fixed a bug preventing some Brutus variants from spawning.
Fixed vanilla lance selection bug in Burden of Proof contract that could cause APCs to spawn as escorts for a convoy of tanks.
Tweaked structure/armor multipliers to slightly debuff low-end vehicles and slighlty buff late-game vehicles.
Small tweaks to evasion and hit.

1.0.5

Hover tanks excluded from enemy convoys!

Fixed invalid movedef for Von Luckners
Adjusted Behemoth and Behemoth-F weapon systems to hopefully elimiate a strange pause before it alpha-strikes.

Striker moved to "Light" category for spawning purposes.
Hunters moved to "Medium" (better at same job for less tonnage.)
